...
public class ReadData {
	...
	private static void collectUserData(String userPath, String split_Sign) throws IOException {
		...
		int[] userIndex = new int[DataInfo.num_of_user];
		
		//System.out.println(DataInfo.num_of_user);
		...
		for(int i = 1; i < DataInfo.num_of_user; i++) {
		...
		for (int i = 0; i != DataInfo.num_of_user; i++) {
			...
			DataInfo.userlist[i] = new int[number];
			
			int p = 0;
			for(int j = first; j < last; j++) {
				DataInfo.userlist[i][p] = user_record[j];
				p++;
			}
		}
	...
	private static void collectItemData(String itemPath, String split_Sign) throws IOException {
		...
		int[] itemIndex = new int[DataInfo.num_of_item];
		
		//System.out.println(DataInfo.num_of_item);
		...
		for(int i = 1; i < DataInfo.num_of_item; i++) {
		...
		for (int i = 0; i != DataInfo.num_of_item; i++) {
			...
			int number = last - first;
			DataInfo.itemlist[i] = new int[number];
			
			int p = 0;
			for(int j = first; j < last; j++) {
				DataInfo.itemlist[i][p] = item_record[j];
				p++;
			}
		}
	}
