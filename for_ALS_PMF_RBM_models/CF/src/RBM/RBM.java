...
public class RBM {
	...
	public static void initScore() {
		...
		for(int user = 0; user < DataInfo.numUsers; user++) {
			...
			for(int j = 0; j < num; j++) {
				int m = DataInfo.TrainSet[user][j] / 100;
				int r = DataInfo.TrainSet[user][j] % 100;
				moviercount[m][r]++;
			}	
		}
	...
	public static void train() {
		...
		while(loopcount < DataInfo.loops) {
			...
			for(int user = 0; user < DataInfo.numUsers; user++) {
			    ...
			    for(int i = 0; i < num; i++) {
			    	int m = DataInfo.TrainSet[user][i] / 100;
					int r = DataInfo.TrainSet[user][i] % 100;
					moviecount[m]++;
			    ...
			    do {
			    	...
			    	for(int i = 0; i < num; i++) {
			            ...
			            if ((randval -= negvisprobs[m][0]) <= 0.0)
			            	...
			            	negvissoftmax[m] = 4;
			            else if ((randval -= negvisprobs[m][5]) <= 0.0)
			            	negvissoftmax[m] = 5;
			            else if ((randval -= negvisprobs[m][6]) <= 0.0)
			            	negvissoftmax[m] = 6;
			            else if ((randval -= negvisprobs[m][7]) <= 0.0)
			            	negvissoftmax[m] = 7;
			            else if ((randval -= negvisprobs[m][8]) <= 0.0)			        	   
			            	negvissoftmax[m] = 8;
			            else if ((randval -= negvisprobs[m][9]) <= 0.0)
			            	negvissoftmax[m] = 9;
			            else		        	   
			            	negvissoftmax[m] = 10;
			            //*/
