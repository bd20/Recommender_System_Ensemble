...
public class RecordErrors {
	...
	public static void rmse() {
		...
		for(int user = 0; user < DataInfo.numUsers; user++) {
			...
			for(int i = 0; i < trainNumber; i++) {
				int item = DataInfo.TrainSet[user][i] / 100;
				int rate = DataInfo.TrainSet[user][i] % 100;
			...
			for(int i = 0; i < trainNumber + testNumber; i++) {
				int item;
				if(i < trainNumber)
					item = DataInfo.TrainSet[user][i] / 100;
				else
					item = DataInfo.TestSet[user].get(i - trainNumber) / 100;
				for(int h = 0; h < DataInfo.totalFeatures; h++) {
			...
			for(int i = 0; i < testNumber; i++) {
				int item = DataInfo.TestSet[user].get(i) / 100;
				int rate = DataInfo.TestSet[user].get(i) % 100;
				...
				//WriteData.writeCsvFile("rbmPred.csv", user, item, predict);
	...
	public static void rmseWithWrite() {
		double nrmse = 0, prmse = 0;
		int tc = 0,pc = 0; 
