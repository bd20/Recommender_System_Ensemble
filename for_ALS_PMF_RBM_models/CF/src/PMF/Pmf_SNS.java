...
public class Pmf_SNS {
	
	@SuppressWarnings("rawtypes")
	static HashMap<Pair, Double> map = new HashMap<Pair, Double>();
	
	@SuppressWarnings("rawtypes")
	public static Stack[] friends = new Stack[DataInfo.userNumber];
	
	static String trainPath  = new String("tr_1_8_1.txt");  
	...
	static void readTrainData(String trainPath, String split_Sign) throws NumberFormatException, IOException
	{
		...
		while(buffRead.ready()) {
			...
			map.put(new Pair<Integer, Integer>(user, item), rating);
	...
	static void readTestData(String trainPath, String split_Sign) throws IOException {
	...
	static double predict(int userId, int itemId) {
		...
		double sumOfsim = 0;
		double sumOfres = 0;
		for(int i = 0; i < friends[userId].size(); i++) {
			int friend = (Integer) friends[userId].get(i);
			if(map.containsKey(new Pair<Integer, Integer>(friend, itemId))) {
				double sim = User_Sim.sim(userId, friend);
				sumOfsim += sim;
				sumOfres += sim * (map.get(new Pair<Integer, Integer>(friend, itemId)) - DataInfo.mean_rating);
			}
		}
		double res = pre + (sumOfres) / (1 + sumOfsim);
	...
	public static void update_one() {
		for (int i = 0; i < DataInfo.trainNumber; i++) {
			...
			double vary = rate - predict(user, item);;
